  ...
  if randomUnit is None:
   ...
   if mapFileName == None:
    if randomInit:
     ...
     for e in vShape:
      somShape.append(e)
     self.M = numpy.random.uniform(mininpvalue[0], maxinpvalue[0], (self.X,self.Y,self.Z,1))
     for e in zip(mininpvalue[1:],maxinpvalue[1:]):
      self.M = numpy.concatenate( (self.M,numpy.random.uniform(e[0],e[1],(self.X,self.Y,self.Z,1))), axis=3 )
    else:
